...
```{r}
els %>% select(stu_id, bys34a, f3ern2011, f1sex) %>% as_factor() %>% head(10)
```
...
- Map hours/week on homework to the x-axis
- Map 2011 income to the y-axis
- Additionally, if we are creating a scatterplot of homework time (x-axis) and income (y-axis), we might use sex to define the color of each point
...
els %>% select(bys34a) %>% filter(bys34a<0) %>% count(bys34a) %>% as_factor()
...
els_v2 %>% count(f1mothed, f1fathed) %>% as_factor()
```
...
```{r}
els_parphd <- els_v2 %>% filter(f1mothed==8, f1fathed==8)
```
...
- This works:
```{r, warning = FALSE}
ggplot(data= els_parphd, aes(x = hw_time, y = earn2011, color = as_factor(f1sex))) + geom_point()
```
</details>
<br>
...
ggplot(data=els_v2) + geom_smooth(mapping = aes(x = hw_time, y = earn2011, group=as_factor(f1sex)))
```
...
ggplot(data=els_v2) + geom_smooth(mapping = aes(x = hw_time, y = earn2011, linetype=as_factor(f1sex)))
```
...
ggplot(data=els_v2) + geom_smooth(mapping = aes(x = hw_time, y = earn2011, color=as_factor(f1sex)))
```
...
```{r}
ggplot(data= els_v2) + 
  geom_point(mapping = aes(x = hw_time, y = earn2011, color = as_factor(f1sex))) + 
  geom_smooth(mapping = aes(x = hw_time, y = earn2011, linetype = as_factor(f1sex))) +
  xlim(c(0,21)) + ylim(c(0,100000))
```
...
__The `scale_x_continuous()` and `scale_y_continuous()` functions__:
...
<br>
__The `scale_color_brewer()` function__:
...
To customize the display of the plot, `ggplot` offers several [preset themes](https://ggplot2.tidyverse.org/reference/ggtheme.html), including:
...
<br>
We can also use [`theme()`](https://ggplot2.tidyverse.org/reference/theme.html) to customize specific components of the plot.
