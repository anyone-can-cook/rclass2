...
els %>% 
  filter(bynels2m > 0) %>% 
  mutate(f1sex = as_factor(f1sex)) %>% 
  ggplot(mapping = aes(x = bynels2m, y = f3ern2011, color = f1sex)) +
  geom_point()
```
...
els %>% 
  filter(bynels2m > 0) %>% 
  mutate(f1sex = as_factor(f1sex)) %>% 
  ggplot(mapping = aes(x = bynels2m, y = f3ern2011)) +
  geom_point()
```
...
```{r, echo = F}
els %>% 
  mutate(byrace = as_factor(byrace)) %>% 
  ggplot(mapping = aes(y = byrace)) +
  geom_bar()
```
