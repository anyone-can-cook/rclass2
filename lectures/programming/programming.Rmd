...
- Using `print()` to print a single object `z` (_`print()` also outputs element number `[1]` while `writeLines()` does not_)
...
CRYSTAL - DEFINE WHAT A CONDITION IS

Any statement that has a length of `1` and can evaluate to either `TRUE` or `FALSE` can be used as the condition:
...
print_hello(x = "Ozan Jaquette", y = "01/16/1979")
  ...
  print_hello(x = "Ozan Jaquette", y = "01/16/1979") %>% str() # investigate what the function returns
...
print_hello(x = "Sumru Jaquette-Nasiali", "2019/04/05") # this does not
```
...
- Just how functions can take inputs (i.e., arguments), functions can also return values as output
