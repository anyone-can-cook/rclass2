...
<br>
When you run R code in an **R markdown file**, the working directory is the directory that your `.Rmd` file is in:
...
- Usually, the `.` and trailing `/` in relative paths are not mandatory
...
- Like folder structure, there is no absolute right or wrong way to organize your R script
...
- Below is one way you can organize your script
...
```{r}
# Read in at most 5 rows (if available)
...
Single R objects can be saved to `.RDS` files using `saveRDS()` and loaded again using `readRDS()`.
